...
import { adminAccountAddress, endpoint } from "../Config/vars";
import { ContextStore } from "../Context/ContextStore";

const Checkout = ({ history }) => {
    const deliveryFee = 60;
    const { contextStore, setContextStore } = useContext(ContextStore);
    const { account, cart, cartTotal, erc20, balance } = contextStore;
    ...
    const onClickPlaceOrder = () => {
        ...
        if (account && cart.length > 0) {
            ...
            if (Number(balance) > Number(total)) {
                erc20.transfer(adminAccountAddress, total).then((result) => {
                    let txId = result.hash;
                    let productCids = cart.map((product) => product.cid);
                    axios
                        .post(`${endpoint}/api/order`, { productCids, txId })
                        .then((res) => {
                            console.log(res.data);
                            erc20.balanceOf(account).then(balance => {
                                setContextStore({...contextStore, balance: balance.toString(),  cart: [],
